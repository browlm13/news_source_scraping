...
class Coo_Matrix:
	...
	# attempt to find synonyms for a word: word_chosen
	# error = []	(word_i, error_i)
	# for each non stop word
	# 	(ignore two chosen words in each iteration)
	#		numpy.delete(arr, obj, axis=None)[source]
	#	normalize both rows values (scale hights based on mean)
	#		numpy.mean
	#	error_i = #abs() sum() if word_chosen_row - word_i_row
	#		numpy.subtract


	# hint: M[i,:], to access a row
	def lowest_normalized_error(self,word):
		all_errors = []
		all_rows = self.A.tolist()		# Takes way too long!!!!

		#normalize chosen row
		chosen_index = self.w2id[word]
		chosen_row = all_rows[chosen_index]
		mu_chosen = np.mean(chosen_row)
		chosen_row_norm = np.divide(chosen_row,mu_chosen)

		index = 0
		for row in all_rows:

			if index != chosen_index:
				#copy chosen normalized row
				cp_chosen_row_norm = np.copy(chosen_row_norm)

				#normalize row
				mu = np.mean(row)
				row_norm = np.divide(row, mu)

				"""
				try:
					#remove selected words
					#cp_chosen_row_norm = np.delete(cp_chosen_row_norm, chosen_index)
					#cp_chosen_row_norm = np.delete(cp_chosen_row_norm, index)
					#row_norm = np.delete(row_norm, chosen_index)
					#row_norm = np.delete(row_norm, index)

					#calculate error
					subtracted_rows = np.subtract(cp_chosen_row_norm, row_norm)
					error = np.mean(np.absolute(subtracted_rows))
		...
		sorted_indexs_error_tuples = sorted(all_errors, key=lambda x: x[1])
		indexs, error = zip(*sorted_indexs_error_tuples)
		ranked_words = [self.id2w[i] for i in indexs]

		# remove short and stop words
		filtered_ranked_words = normalizers.remove_short_and_stopwords(ranked_words)
